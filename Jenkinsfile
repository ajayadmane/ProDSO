    ...
    stages {
       ...
       // }
        stage('Install Dependencies') {
            steps {
                sh 'cd $WORKSPACE'
                sh 'rm -rf IACSD-Project'
                git branch: "master",
                    url: "https://github.com/ajayadmane/ProDSO.git"
                sh 'ls'
            }
        }
            ...
            parallel {
                ...
                stage('Image Security') {
                    steps {
                        ...
                        subject: "${env.JOB_NAME} - Build # ${env.BUILD_NUMBER} - success", mimeType: 'text/html', to: "maskoff.ajayraj@gmail.com"
                    }
                }
            }
        }
        ...
        stage('Artifacts for Dependencies') {
            parallel {
                stage('Dependency Check') {
                    steps {
                        sh 'wget https://raw.githubusercontent.com/ajayadmane/ProDSO/master/dc.sh'
                        sh 'chmod +x dc.sh'
                        sh './dc.sh'
